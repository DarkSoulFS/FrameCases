...
public class GUI {
    ...
    public static Inventory inventory(Player player) {
        ...
        for (String items : instance.getConfig().getConfigurationSection("gui.items").getKeys(false)) {
            try {
            	ConfigurationSection item = instance.getConfig().getConfigurationSection("gui.items." + items);
